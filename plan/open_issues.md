...
## The ESP32-P4's preemption tick runs at 96 Hz, not the 100 Hz it is asked for

**Found 2026-09-16 by 34.1's `clocks` command**
(`plan/phase34_esp32p4_pll_bringup.md`), which is the first instrument this
board has ever had for the question. Parked rather than fixed on the spot
because the *cause* of the bias needs its own diagnosis and 34.1 is a
read-only milestone; it must be fixed before 34.4, which uses the ticker's
figure to answer what clocks the CLINT.

**The chain, measured end to end on the board:**

| | figure | how |
|---|---|---|
| CLINT rate, `ticker.c`'s 2 ms window at boot | 41 626 307 Hz | derived, and **4.07 % high** |
| CLINT rate, 1 s window from `clocks` | 40 000 029 Hz | two samples, 16 Hz apart |
| predicted tick rate from the first | 96.09 Hz | 100 / 1.0407 |
| **measured tick rate** | **95.99 Hz** | 6192 ticks over 64.51 s |

The prediction and the measurement agree to 0.1 %, and the residual is in the
direction and of the size `plan/phase27_esp32p4_bringup.md:1384` already
attributes to `ticker_next()`'s relative rearm. So the tick is slow *because*
the boot measurement is high, and nothing else is involved.

The 1 s figure is the true one: 40 MHz to within 1 ppm is what a
crystal-derived clock looks like, and it is repeatable. Note it does **not**
yet say whether the CLINT follows the CPU or the crystal — today those are
both 40 MHz. 34.4 separates them.

**Why it probably appeared without anyone touching the ticker.** Phase 27
measured this tick at 99.8742 Hz over 616 seconds (E4), and that was a
RAM-resident image. `kernel/ticker.c:272` chose its 2 ms window as *"long
enough to divide by and short enough not to be noticed at boot"* — a
measurement whose accuracy depends on the cost of the code inside its own
window, and phase 32 moved that code into the XIP flash window. That is the
leading explanation and it is **not verified**: the 99.87 Hz figure comes
from a different image, not from a before/after pair on this one.

**Worth doing:** find the systematic term rather than lengthening the window
until it hides. The bias is +4.07 % of 2000 µs, which is 81 µs — far too
large for read quantisation (±1 systimer µs is 500 ppm) and far too
repeatable for noise: three boots gave 41 630 293 Hz to the hertz, and a
fourth, after a rebuild that shifted `.text`, gave 41 626 307 Hz. Something in
`arch_ticker_init()`'s window costs a fixed 81 µs that it does not account
for; the systimer's UPDATE/VALID handshake in `time_get_us()` is the first
place to look. The RP2350 arm measures the same way and is not implicated —
its own rate was checked independently.
