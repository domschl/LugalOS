...
itself down to two fids for exactly that reason. FUSE(..., nothreads=True)
(see cli.py) is what actually makes this safe: it forces single-threaded
dispatch, so at most one Operations method ever runs at a time and the
Session never needs its own locking. Giving each mount its own fid pool
for real concurrency is future work, not something to grow into here.
...
class P9FS(Operations):
    ...
    def statfs(self, path):
        ...
        # -f` don't error, not a real measurement.
        return {
            "f_bsize": 4096,
            "f_frsize": 4096,
            "f_blocks": 1 << 20,
