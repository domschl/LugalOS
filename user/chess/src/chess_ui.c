...
/* J5 (plan/phase10_chess_completion.md): platform-specific raw byte I/O for
 * chess_uci_run() below -- RP2350's dedicated ACM2/EP6 (drivers/usb_cdc.c)
 * vs QEMU's single virtio-console link, temporarily borrowed from
 * background 9P (kernel/device.h's dev_get(), fs/p9_link.h's register/
 * unregister_background()). See chess_uci_run()'s own comment. */
...
/* ------------------------------------------------------------------ *
 * J5 (plan/phase10_chess_completion.md): the UCI-over-a-dedicated-    *
 * wire protocol loop -- standard UCI only (uci/isready/setoption/     *
 * ucinewgame/position/go/quit), no TM1638/ST7735 coupling, no menu    *
 * system. Reuses console_execute_move() (above) for move parsing --   *
 * upstream's uci.c has its own near-identical parse_and_make_move(),  *
 * not worth a second copy of the same ~25 lines.                      *
 *                                                                      *
 * Deliberately does NOT rely on search.c's own embedded                *
 * printf("bestmove ...")/printf("info depth ...") lines (search.c:    *
 * 668/765-773/861) -- those go through cprintf(), i.e. wherever the    *
 * system console is currently bound (ACM0/lsh's own terminal on       *
 * RP2350), never this function's own byte stream. Rebinding the       *
 * console to this wire for the session (console_bind_device()) was    *
 * the other way to make that reach the right port, and was rejected:  *
 * it would silence the interactive shell's own output on ACM0/UART0   *
 * for as long as chess-uci runs, which defeats the entire reason this *
 * milestone gave chess its own dedicated wire in the first place.     *
 * Every line below is built here instead, from the same                *
 * g_search_best_move/g_search_score/g_search_depth state search_       *
 * progress_callback() (above) already captures for every other front  *
 * end -- console_engine_reply()'s "Engine plays: %s" already            *
 * establishes this exact precedent for chess_console_run().            *
 * ------------------------------------------------------------------ */

#if defined(CONFIG_BOARD_RP2350)
/* ACM2/EP6 (drivers/usb_cdc.c) -- a dedicated wire, never shared with
 * usbnet's P9, so there is nothing to suspend/resume here. The endpoint's
 * hardware stays unconfigured (port present but silent) until the first
 * call below, matching CONFIG_ENABLE_CHESS's own "not active by default"
 * contract (usb_cdc.h). */
static void uci_io_open(void) {
    usb_cdc_uci_ensure_init();
}
static void uci_io_close(void) {
    /* Nothing to release -- ACM2 stays armed for the rest of this boot,
     * the same lifetime EP2/EP4 already have. A later chess-uci call
     * reuses it rather than reconfiguring from scratch. */
}
static uint32_t uci_io_read(uint8_t *buf, uint32_t max) {
    return usb_cdc_uci_read(buf, max);
}
static int uci_io_write(const uint8_t *buf, uint32_t len) {
    return usb_cdc_uci_write(buf, len);
}
#else
/* QEMU has no USB CDC at all -- borrows the single virtio-console link,
 * which kernel/main.c registers for background 9P at boot (dev_vconsole,
 * DEV_F_BACKGROUND_9P). Suspended for the duration of this session so the
 * background 9P poll and this function's own reads don't race over the
 * same byte ring (drivers/virtio_console.c's g_rx_ring) -- p9share's own
 * off/on toggle is the precedent for suspending a background link like
 * this. Real hardware never hits this: ACM2 is dedicated, see above. */
static p9_link_t *g_uci_qemu_link = NULL;
static void uci_io_open(void) {
    g_uci_qemu_link = (p9_link_t *)dev_get("vconsole", DEV_KIND_P9LINK);
    if (g_uci_qemu_link != NULL) {
        p9_link_unregister_background(g_uci_qemu_link);
    }
}
static void uci_io_close(void) {
    if (g_uci_qemu_link != NULL) {
        p9_link_register_background(g_uci_qemu_link);
        g_uci_qemu_link = NULL;
    }
}
static uint32_t uci_io_read(uint8_t *buf, uint32_t max) {
    return virtio_console_read_raw(buf, max);
}
static int uci_io_write(const uint8_t *buf, uint32_t len) {
    return virtio_console_write_raw(buf, len);
}
#endif

/* Manual decimal digits, same reason perft.c's print_u64() and chess_ui.c's
 * own append_uint() (DISPLAY-only, further below) both exist: no snprintf
 * on this freestanding build. A third small copy rather than reaching into
 * the DISPLAY-gated one, since this function must build on every target
 * (QEMU included), not just CONFIG_ENABLE_DISPLAY boards. */
static int uci_append_uint(char *buf, int pos, unsigned v) {
    char tmp[10];
    int n = 0;
    if (v == 0) tmp[n++] = '0';
...
static int uci_append_int(char *buf, int pos, int v) {
    if (v < 0) { buf[pos++] = '-'; v = -v; }
    return uci_append_uint(buf, pos, (unsigned)v);
}
static int uci_append_str(char *buf, int pos, const char *s) {
    while (*s) buf[pos++] = *s++;
    return pos;
}

static void uci_send_line(const char *s) {
    uci_io_write((const uint8_t *)s, (uint32_t)strlen(s));
}

/* Reads one line (bare '\n' or '\r\n' terminated, either tolerated the same
 * way uci.c's own strcspn(line, "\n")/strcspn(line, "\r") strips both) into
 * `line`, blocking a poll at a time until one arrives. Returns the line
 * length; only 'quit' (chess_uci_run()'s own loop) ends a session, matching
 * p9serve's own "no software escape, reset to exit" precedent -- NOT
 * chess_abort_requested()/Ctrl-C, deliberately, unlike every other blocking
 * wait in this file. That function's console_interrupt_requested() half
 * (kernel/console.c) drains the *entire* primary-console UART RX FIFO on
 * every call, discarding every non-Ctrl-C byte -- correct for chess_run()/
 * chess_console_run(), where the human's own keystrokes and that UART are
 * the same stream anyway, but wrong here: chess-uci's whole reason for a
 * dedicated wire (ACM2 on RP2350, J5) is to let an operator keep typing on
 * their own console (ACM0) *while* a UCI session runs on a separate one.
 * Polling chess_abort_requested() from this loop would silently eat that
 * unrelated console's input for as long as chess-uci stays open -- found
 * live, via tests/runner.py's own UCI test failing its final "console
 * responds again after quit" check with the *previous* command's stdin
 * write missing entirely, traced back to this exact drain. A lone '\r' or
 * '\n' with nothing buffered yet is silently absorbed rather than returned
 * as an empty line, so a "\r\n"-terminated command doesn't produce a
 * spurious blank read on the next call. */
static void uci_read_line(char *line, int max_len) {
    int n = 0;
    for (;;) {
        uint8_t c;
        if (uci_io_read(&c, 1) == 0) {
            time_delay_us(2000);
            continue;
        }
        if (c == '\n' || c == '\r') {
        ...
        if (n < max_len - 1) line[n++] = (char)c;
    }
}

/* `position [startpos | fen <fen>] [moves <m1> <m2> ...]`, ported from
 * uci.c's own `position` handler minus its stdio -- parses in place against
 * `line` (the caller's already-null-terminated buffer) rather than copying,
 * same as console.c's own command dispatch above does throughout. */
static void uci_handle_position(Position *pos, char *args) {
    while (*args == ' ') args++;
    ...
    if (strncmp(args, "startpos", 8) == 0) {
        ...
        if (moves_ptr != NULL) {
            fen_end = moves_ptr;
            while (fen_end > args && fen_end[-1] == ' ') fen_end--;
            saved = *fen_end;
            *fen_end = '\0';
        }
        static Position temp_pos;
        parse_fen(&temp_pos, args);
        if (is_position_valid(&temp_pos)) {
            *pos = temp_pos;
        } else {
            uci_send_line("info string Error: Invalid FEN position\n");
        }
        if (fen_end != NULL) *fen_end = saved;
        args = (moves_ptr != NULL) ? moves_ptr : (args + strlen(args));
    }
    ...
    if (moves_ptr != NULL) {
        ...
        while (*moves_ptr != '\0') {
            ...
            move_str[len] = '\0';
            console_execute_move(pos, move_str);
        }
    }
}

/* `go [depth N | movetime N | wtime N btime N]`, ported from uci.c's own
 * `go` handler minus its stdio. wtime/btime use the same /20 budget
 * upstream does (one twentieth of remaining clock per move -- crude but
 * upstream's own choice, not revised here). */
static void uci_handle_go(Position *pos, const char *args) {
    int max_depth = 64;
    int time_limit_ms = -1;
    const char *p;
    if ((p = strstr(args, "movetime")) != NULL) {
        ...
        max_depth = atoi(p + 5);
    } else if (pos->side == WHITE && (p = strstr(args, "wtime")) != NULL) {
        time_limit_ms = atoi(p + 5) / 20;
    } else if (pos->side == BLACK && (p = strstr(args, "btime")) != NULL) {
        time_limit_ms = atoi(p + 5) / 20;
    }
    ...
    char out[192];
    int n = 0;
    n = uci_append_str(out, n, "info depth ");
    n = uci_append_int(out, n, g_search_depth);
    /* g_search_score is already side-to-move-relative, exactly the
     * convention UCI's own "score cp/mate" wants -- unlike tm_format_
     * score_compact() (TM1638/TFT-only, further below), which deliberately
     * un-flips this same value to a stable White POV for display. No
     * un-flipping here: this is the one place the raw engine value is
     * exactly right, unmodified. Same MATE_VALUE/INFINITY_VALUE mate-
     * distance math as that function and as search.c's own embedded (but
     * cprintf-bound, never reaching this wire) "score mate" lines. */
    if (g_search_score >= MATE_VALUE - 1000) {
        int mate_moves = (INFINITY_VALUE - g_search_score + 1) / 2;
        n = uci_append_str(out, n, " score mate ");
        n = uci_append_int(out, n, mate_moves);
    } else if (g_search_score <= -MATE_VALUE + 1000) {
        int mate_moves = (INFINITY_VALUE + g_search_score + 1) / 2;
        n = uci_append_str(out, n, " score mate -");
        n = uci_append_int(out, n, mate_moves);
    } else {
        n = uci_append_str(out, n, " score cp ");
        n = uci_append_int(out, n, g_search_score);
    }
    n = uci_append_str(out, n, " nodes ");
    n = uci_append_int(out, n, (int)nodes_searched);
    n = uci_append_str(out, n, "\n");
    out[n] = '\0';
    uci_send_line(out);

    n = 0;
    n = uci_append_str(out, n, "bestmove ");
    if (best != 0) {
        char buf[6];
        format_move(best, buf);
        n = uci_append_str(out, n, buf);
        make_move(pos, best);
    } else {
        n = uci_append_str(out, n, "0000"); /* UCI's own "no legal move" sentinel */
    }
    n = uci_append_str(out, n, "\n");
    out[n] = '\0';
    uci_send_line(out);
...
/* Does not return until `quit` -- same shape as chess_console_run() above,
 * except that's the *only* way out (see uci_read_line()'s own comment for
 * why Ctrl-C deliberately isn't). Unlike that REPL, nothing here is meant
 * for a human to read interactively; every line is protocol, not prose. */
void chess_uci_run(void) {
    ...
    char line[2048];
    for (;;) {
        uci_read_line(line, sizeof(line));

        if (strcmp(line, "uci") == 0) {
            uci_send_line("id name LugalChess LugalOS\n");
            uci_send_line("id author Antigravity\n");
            uci_send_line("uciok\n");
        } else if (strcmp(line, "isready") == 0) {
            uci_send_line("readyok\n");
        } else if (strcmp(line, "ucinewgame") == 0) {
            parse_fen(&pos, STANDARD_START_FEN);
            clear_tt();
        } else if (strncmp(line, "position", 8) == 0) {
            uci_handle_position(&pos, line + 8);
        } else if (strncmp(line, "go", 2) == 0 && (line[2] == '\0' || line[2] == ' ')) {
            uci_handle_go(&pos, line);
        } else if (strcmp(line, "stop") == 0) {
            /* Nothing to interrupt here specifically -- see console.c's
             * own "stop" no-op above for why (uci_read_line() only reads
             * this once the previous go's search has already returned). */
        } else if (strcmp(line, "quit") == 0) {
            break;
        }
        /* Unrecognized lines are silently ignored, per the UCI spec's own
         * forward-compatibility rule -- a GUI may send commands this
         * minimal implementation doesn't need to act on (e.g. "debug on"). */
    }
